...
export function DashboardCharts({ charts = {}, period = "month" }) {
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-3">
      ...
      <div className="panel p-4">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <PieChart><Pie data={status} dataKey="value" nameKey="name" innerRadius={52} outerRadius={86} paddingAngle={3}>{status.map((_, i) => <Cell key={i} fill={colors[i % colors.length]} />)}</Pie><Tooltip contentStyle={tooltipStyle} /></PieChart>
        </ResponsiveContainer>
      </div>
      <div className="panel p-4 xl:col-span-2">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={trips}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} /><XAxis dataKey="name" tickLine={false} axisLine={false} /><YAxis tickLine={false} axisLine={false} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="value" fill="#10b981" radius={[6, 6, 0, 0]} maxBarSize={42} /></BarChart>
        </ResponsiveContainer>
      </div>
      <div className="panel p-4">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={bookings}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} /><XAxis dataKey="name" tickLine={false} axisLine={false} /><YAxis tickLine={false} axisLine={false} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="value" fill="#2388d9" radius={[6, 6, 0, 0]} maxBarSize={42} /></BarChart>
        </ResponsiveContainer>
      </div>
    </div>
  );
